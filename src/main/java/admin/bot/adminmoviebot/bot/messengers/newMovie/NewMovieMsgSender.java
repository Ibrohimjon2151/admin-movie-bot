...
public final class NewMovieMsgSender implements NewMovieMsgSenderInt {
 ...
 private SendMessage getCategoriesList(Update update) {
  ...
  array[array.length - 1] = Buttons.BTN_BACK;
  array[ids.length - 1] = Buttons.BTN_BACK;
 ...
 public SendMessage movieFunctions(Update update) {
  ...
  switch (message) {
   ...
   case Buttons.BTN_MOVIE_LIST -> {
    sendMessage = getAllMovieCategories(update);
   }
 ...
 @Override
 public SendMessage getAllMovieCategories(Update update) {
 ...
 public List<SendMessage> drawMovieByCategory(Update update) {
  ...
  if (data.equals(Buttons.BTN_BACK)) {
   ...
   List<Movie> movieList = movieService.getMoviesByCategoryId(Long.valueOf(data));
   if (movieList != null) {
    ...
    // TANLANGAN CATEGORY BO'YIHA HECH NARSA TOPILMADI
   }
  }
  return List.of();
 }
